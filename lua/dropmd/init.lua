...
function M.setup(opts)
  ...
  if M.opts.rename_pattern and not M.opts.rename_fn then
    M.opts.rename_fn = (function(pattern)
    ...
    end)(M.opts.rename_pattern)
  end

  if not M.opts.formatter_fn then
    M.opts.formatter_fn = function(filename, path)
  ...
  if not ok then
    vim.notify("[dropmd.nvim] 'DropPre' is not supported in this environment.", vim.log.levels.WARN)
  end
end
